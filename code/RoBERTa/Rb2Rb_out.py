...
def calculate_bleu(data, model,epoch):
    ...
    with open("Epoch"+str(epoch)+"preds.txt", "w", encoding="utf-8") as f:
      for i in pred_trgs:
        f.write(i+"\n")
    with open("Epoch"+str(epoch)+"given.txt", "w", encoding="utf-8") as f:
      for i in trgs:
        f.write(i+"\n")
    outcsv={'review':srcs,'predicted reply':pred_trgs,'trg reply':trgs}
    outcsvdf=pd.DataFrame(outcsv)
    outcsvdf.to_csv("Epoch"+str(epoch)+"Whole_Review_Reply.csv",index=False)
    end_time=time.time()
...
def main():
    ...
    model=EncoderDecoderModel.from_pretrained("/scratch/ex-fhendija-1/caoyuecc/Roberta/checkpoints/Batch32/checkpoint-5989")
    ...
    calculate_bleu(test_data,model,1)
