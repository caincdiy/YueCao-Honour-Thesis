...
def save_checkpoint(opts, experiment_name, encoder, decoder, encoder_optim, decoder_optim,
    ...
    #checkpoint_path = '/checkpoints/single_wc%s_hs%s_ln%s_dp%.1f_rslckt%s/%s_acc_%.2f_loss_%.2f_step_%d.pt' % (
    ...
    checkpoint_path='checkpoints/2o3/'+"accuracy"+str(total_accuracy)+'_step'+str(global_step)+'_checkpoint.pt'
    ...
    if total_accuracy>55:
        torch.save(checkpoint, checkpoint_path)
    else:
        print("global step"+str(global_step)+" accuracy smaller than 55, didn't save the checkpoint")
        checkpoint_path="accuracy smaller than 55, didn't save the checkpoint"
