...
args = Namespace(
    glove_filename='./data/RRGenFormat/1o3RRGendata/glove.6B.100d.txt',
    test_filename='./data/RRGenFormat/1o3RRGendata/test_data.txt',
    train_filename='./data/RRGenFormat/1o3RRGendata/train_data.txt',
    valid_filename='./data/RRGenFormat/1o3RRGendata/valid_data.txt',
    cate_filename='./data/RRGenFormat/1o3RRGendata/app_category.txt',
    keyword_filename='./data/RRGenFormat/1o3RRGendata/keyword_dict.json',
    feature_vec_size = 20
)
...
class Dataset():
    def __init__(self, data_file, max_vocab_size=50, src_vocab=None, tgt_vocab=None, rate_vocab=None, cate_vocab=None, senti_vocab=None):
        ...
        self.src_sents, self.tgt_sents, self.rate_sents, self.app_cates, self.senti_sents = self.load_sents(self.data_file)#这里读取数据并且把它们打包成了vector
    ...
    def load_sents(self, fpath):
        ...
        with open(fpath) as file:
            for sent in tqdm(file.readlines()):
                ...
                src_sent = terms[4]
                tgt_sent = terms[5]
                src_tokens = [token for token in src_sent.split()] #把输入的句子转化成list
                tgt_tokens = [token for token in tgt_sent.split()]
                src_sents.append(src_tokens) #src是list 包含list
